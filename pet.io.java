...
public class Main {
    public static void createFiles() throws IOException {
        FileWriter domesticFile = new FileWriter("Домашние_животные");
        domesticFile.write("Собаки\nКошки\nХомяки");
        domesticFile.close();

        FileWriter livestockFile = new FileWriter("Вьючные_животные");
        livestockFile.write("Лошади\nОслы\nВерблюды");
        livestockFile.close();
    }

    public static void mergeFiles() throws IOException {
        BufferedReader domesticReader = new BufferedReader(new FileReader("Домашние_животные"));
        BufferedReader livestockReader = new BufferedReader(new FileReader("Вьючные_животные"));
        BufferedWriter mergedWriter = new BufferedWriter(new FileWriter("Друзья_человека"));
    ...
    public static void createDirectory() {
        ...
        File fileToMove = new File("Друзья_человека");
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    System.out.print("Введите имя животного, для которого нужно добавить команду: ");
                    String animalNameToAddCommand = scanner.nextLine();
                    System.out.print("Введите команду: ");
                    String newCommand = scanner.nextLine();
                    for (Animal animal : registry.animals) {
                        if (animal.name.equalsIgnoreCase(animalNameToAddCommand)) {
                            if (animal instanceof DomesticAnimal) {
                                ((DomesticAnimal) animal).addCommand(newCommand);
                            } else if (animal instanceof LivestockAnimal) {
                                ((LivestockAnimal) animal).addCommand(newCommand);
                            }
                            break;
                        }
                    }
